...
public abstract class MixinMobEntity extends LivingEntity {
	...
	public void immunizePiglin(PlayerEntity player, Hand hand, CallbackInfoReturnable<ActionResult> cir) {
		...
		if (item.isOf(Items.GOLDEN_APPLE) && (zombified && ((LivingEntity)self).hasStatusEffect(StatusEffects.WEAKNESS) || can && !((ZombImmunizableEntity)self).fabrication$isZombImmune())) {
			item.decrement(1);
			this.playSound(SoundEvents.ENTITY_PLAYER_BURP, 1, this.getRandom().nextFloat() * 0.5F);
			if (zombified) self = this.convertTo(((GetPreZombified)self).fabrication$getPreZombifiedType(), true);
